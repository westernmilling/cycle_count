...
describe CycleCountDecorator, type: :decorator do
  let(:decorator) do
    build(:cycle_count,
  ...
  let(:requested_date) { Faker::Date.forward(30) }
  ...
  describe '#formatted_requested_date' do
    subject { decorator.formatted_requested_date }

    it { is_expected.to eq(requested_date.strftime('%m/%d/%Y')) }
  end
end
